With nodeCount AS (
 SELECT g.id, count(DISTINCT n.id) AS nodecount
      FROM nodes n, plasco_grid_metric_30 g
  WHERE st_within(n.geom, st_transform(g.geom,4326)) AND n.event = 'plasco' AND n."timestamp" > '2016-12-20 00:00:00-04' AND n.timestamp < '2017-01-18' 
  GROUP BY g.id
  ORDER BY g.id
     ),
 wayCount AS(
      SELECT g.id, count(DISTINCT w.way_id) AS waycount
      FROM way_nodes_centroids w, plasco_grid_metric_30 g
  WHERE st_within(w.centroid, st_transform(g.geom,4326)) AND w.event = 'plasco' AND w."timestamp" > '2016-12-20 00:00:00-04' AND w.timestamp < '2017-01-18' 
  GROUP BY g.id
  ORDER BY g.id
      ),
 allcounts AS(
...
maxVersions AS (
     ...
     From nodes n
     Where n.event = 'plasco' AND n."timestamp" > '2016-12-20 00:00:00-04' AND n.timestamp < '2017-01-18' 
     Group by n.id
     ),
averageVersions AS(
 SELECT g.id, round(avg(v.maxversion), 2) AS avgversion
      FROM nodes n 
          ...
          plasco_grid_metric_30 g
  WHERE st_within(n.geom, st_transform(g.geom,4326)) 
  AND n.event = 'plasco' AND n."timestamp" > '2016-12-20 00:00:00-04' AND n.timestamp < '2017-01-18' 
  GROUP BY g.id
  ORDER BY g.id
    ),
  -- Number of users
 ...
 wayuserCount AS(
      ...
      FROM plasco_grid_metric_30 g, way_nodes_centroids w
  WHERE st_within(w.centroid, st_transform(g.geom,4326)) AND w.event = 'plasco' AND w."timestamp" > '2016-12-20 00:00:00-04' AND w.timestamp < '2017-01-18' 
  GROUP BY g.id
  ORDER BY g.id
      ), 
...
nodeattributecount AS(    
    ...
    WHERE st_within(n.geom, st_transform(g.geom,4326)) 
  		AND n.event = 'plasco' 
     	AND n."timestamp" > '2016-12-20 00:00:00-04' AND n.timestamp < '2017-01-18'  
