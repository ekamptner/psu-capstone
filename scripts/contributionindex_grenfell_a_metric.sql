With nodeCount AS (
 SELECT g.id, count(DISTINCT n.id) AS nodecount
      FROM nodes n, grenfell_grid_metric_30 g
  WHERE st_within(n.geom, st_transform(g.geom,4326)) AND n.event = 'grenfell' AND n."timestamp" > '2017-06-14 00:00:00-04'
  GROUP BY g.id
  ORDER BY g.id
     ),
 wayCount AS(
      SELECT g.id, count(DISTINCT w.way_id) AS waycount
      FROM way_nodes_centroids w, grenfell_grid_metric_30 g
  WHERE st_within(w.centroid, st_transform(g.geom,4326)) AND w.event = 'grenfell' AND w."timestamp" > '2017-06-14 00:00:00-04'
  GROUP BY g.id
  ORDER BY g.id
      ),
 allcounts AS(
...
averageVersions AS(
 ...
 wayuserCount AS(
      ...
      FROM grenfell_grid_metric_30 g, way_nodes_centroids w
  WHERE st_within(w.centroid, st_transform(g.geom,4326)) AND w.event = 'grenfell' AND w."timestamp" > '2017-06-14 00:00:00-04'
  GROUP BY g.id
  ORDER BY g.id
      ), 
...
From grenfell_grid_metric_30 g 
